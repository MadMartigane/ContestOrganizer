...
import type { NBAScheduleConfig } from "./nba.types";

describe("NBA Algorithm Registry", () => {
  const createMockTournament = (teamCount: number): Tournament => {
  ...
  const defaultConfig: NBAScheduleConfig = {
    maxGamesPerTeam: 82,
  ...
  describe("runWithAlgorithm", () => {
    ...
    it("should throw error for invalid algorithm name", () => {
      const tournament = createMockTournament(4);
      expect(() =>
        runWithAlgorithm("invalid" as "minimax", tournament, defaultConfig)
  ...
  describe("getAlgorithmInfo", () => {
    ...
    it("should throw error for invalid algorithm name", () => {
      expect(() => getAlgorithmInfo("invalid" as "minimax")).toThrow(
