...
describe("ErrorMessage", () => {
  ...
  it("should render with message attribute", () => {
    ...
    expect(element._renderRoot.innerHTML).toContain("Test error message");
    expect(element._renderRoot.innerHTML).toContain("mad-callout");
  });
  it("should show home button by default", () => {
    const element = document.createElement("error-message");
    element.setAttribute("message", "Error");
    document.body.appendChild(element);
    expect(element._renderRoot.innerHTML).toContain("Retour à l'accueil");
  });
  it("should hide home button when go-home-button is false", () => {
    ...
    document.body.appendChild(element);
    expect(element._renderRoot.innerHTML).not.toContain("Retour à l'accueil");
  });
  it("should update when message attribute changes", async () => {
    const element = document.createElement("error-message");
    element.setAttribute("message", "Initial");
    document.body.appendChild(element);
    expect(element._renderRoot.innerHTML).toContain("Initial");
    element.setAttribute("message", "Updated");
    await new Promise((resolve) => setTimeout(resolve, 10));
    expect(element._renderRoot.innerHTML).toContain("Updated");
  });
});
