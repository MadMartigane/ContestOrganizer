...
export class AppStatusNews extends BaseElement {
  ...
  private declare _state: Signal<ComponentState>;
  ...
  protected _setupProperties(): void {
    this._state = new Signal<ComponentState>("loading");
   ...
   */
  private _renderReadyState(): TemplateResult {
    return html`
  ...
  private _renderSections(): TemplateResult {
    ...
    return html`
      ${statusData.sections.map((section: StatusSection) => this._renderSection(section))}
    `;
  }
   ...
   */
  private _renderSection(section: StatusSection): TemplateResult {
    ...
    return html`
      <mad-card appearance="outlined" data-section-id="${section.id}" role="listitem">
        <div 
          ...
          @keydown=${(e: KeyboardEvent) => this._handleSectionKeydown(e, section.id)}
        >
   ...
   */
  private _renderIndicator(type: StatusSection["type"]): TemplateResult {
   ...
   */
  private _renderTables(tables: StatusSection["tables"]): TemplateResult {
   ...
   * @returns {void}
   */
  private _handleSectionKeydown(event: KeyboardEvent, sectionId: string): void {
   ...
   */
  private _renderVersionBadge(): TemplateResult {
    return html`
